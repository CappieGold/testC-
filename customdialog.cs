    ...
    public class WinKioskCustomInstaller : Installer
    {
        ...
        public override void Install(IDictionary stateSaver)
        {
            ...
            try
            {
                ...
                string useExistingAccount = Context.Parameters["USEEXISTINGACCOUNT"];
                string username = Context.Parameters["USERNAME"];
                string password = Context.Parameters["PASSWORD"];

                targetDir = targetDir.TrimEnd('\\', ' ');

                if (useExistingAccount == "1" && !string.IsNullOrEmpty(username))
                {
                    LogEvent($"Installing for existing user: {username}");
                    StoreUsernameInRegistry(username);
                    string sidString = GetUserSid(username);
                    SetCustomShellAndPolicies(username, string.Empty, sidString, targetDir);
                    EnableAutoLogon(username, string.Empty);
                }
                else if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(password))
                {
                    LogEvent($"Creating new user: {username}");
                    StoreUsernameInRegistry(username);
                ...
                {
                    throw new InstallException("Invalid user selection. Please select an existing user or create a new one.");
                }
            }
            catch (Exception ex)
            {
        ...
        public override void Uninstall(IDictionary savedState)
        {
            ...
            try
            {
                ...
                {
                    LogEvent($"Uninstalling for user: {username}");
                    bool isExistingUser = Context.Parameters["USEEXISTINGACCOUNT"] == "1";
                ...
                {
                    LogEvent("Uninstalling for default user: Kiosk");
                    DeleteUser("Kiosk");
                    DisableAutoLogon();
                }
            }
